...
class FirestoreUserStore implements UserStore {
  ...
  Future<UserSnapshot> load(String uid) async {
    ...
    try {
      ...
      final bookmarks = (results[0] as QuerySnapshot<Map<String, dynamic>>)
          .docs
          .map((d) => d.id)
          .toSet();
      ...
      for (final doc
        ...
        progress[doc.id] = ReadingProgress.fromJson({
          ...data,
          'updatedAt':
              stamp is Timestamp ? stamp.toDate().toIso8601String() : stamp
        });
      }
      final settings =
  ...
  Future<void> saveBookmarks(
    ...
    for (final id in ids) {
      batch.set(ref.doc(id),
          {'mangaId': id, 'updatedAt': FieldValue.serverTimestamp()});
    }
    await batch.commit();
  }
  ...
  Future<void> saveAdultContent(String uid, bool enabled) async {
    ...
    await _db
        .collection('users')
        .doc(uid)
        .set({'adultContent': enabled}, SetOptions(merge: true));
  }
