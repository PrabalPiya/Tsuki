...
class AniListMetadataProvider implements MetadataProvider {
  ...
  static const fields =
      r'''id idMal title { romaji english native } synonyms description(asHtml: false)
  ...
  Future<List<Manga>> search(String query, {required bool includeAdult}) async {
    ...
    final page = (response.data?['data'] as Map<String, dynamic>?)?['Page']
  ...
  Future<Manga?> getById(String id) async {
    ...
    final response = await _client.post<Map<String, dynamic>>('', data: {
      'query': document,
      'variables': {'id': value}
    });
    final media = (response.data?['data'] as Map<String, dynamic>?)?['Media']
        as Map<String, dynamic>?;
  ...
  Future<List<Manga>> _browse(
    ...
    final document = '''query BrowseManga {
      Page(page: 1, perPage: 24) { media(type: MANGA, isAdult: false, $seasonClause sort: $sort) { $fields } }
    }''';
    final response =
        await _client.post<Map<String, dynamic>>('', data: {'query': document});
    final page = (response.data?['data'] as Map<String, dynamic>?)?['Page']
        as Map<String, dynamic>?;
    return (page?['media'] as List? ?? const [])
        .cast<Map<String, dynamic>>()
