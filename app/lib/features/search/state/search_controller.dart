...
class SearchController extends StateNotifier<SearchState> {
  ...
  Future<void> _run(String query, int generation, bool submitted) async {
    state =
        state.copyWith(loading: true, submitted: submitted, clearError: true);
    try {
      final result =
          await _repository.search(query, includeAdult: _includeAdult());
      if (generation == _generation) {
        state = state.copyWith(
            results: result, loading: false, submitted: submitted);
      }
    } catch (_) {
      if (generation == _generation) {
        state = state.copyWith(
            loading: false, error: 'Search is unavailable. Try again.');
      }
    }
  }
