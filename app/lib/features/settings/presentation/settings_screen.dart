...
class SettingsScreen extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return Scaffold(
        ...
        body: ListView(
            ...
            children: [
              _SectionCard(title: 'Reading preferences', children: [
                _SwitchRow(
                    ...
                    onChanged:
                        ref.read(userLibraryProvider.notifier).setAdultContent),
              ]),
                  ...
                  children: session.isLocalProfile
                      ? [
                          const _InfoRow(
                              ...
                              subtitle:
                                  'Bookmarks and progress stay on this phone.'),
                          _ActionRow(
                              icon: Icons.delete_outline_rounded,
                              title: 'Clear local data',
                              subtitle:
                                  'Remove bookmarks, progress, and settings.',
                              destructive: true,
  ...
  Future<void> _confirmDelete(BuildContext context, WidgetRef ref) async {
    final accepted = await _showConfirmDialog(
        ...
        message:
            'This permanently deletes bookmarks, progress, settings, and the authentication account.',
        confirmLabel: 'Delete',
        destructive: true);
    if (accepted == true) {
  ...
  Future<void> _confirmClearLocal(BuildContext context, WidgetRef ref) async {
    final accepted = await _showConfirmDialog(
        ...
        title: 'Clear local data?',
        message:
            'This removes all bookmarks, reading progress, and settings from this device.',
        confirmLabel: 'Clear',
        destructive: true);
    if (accepted == true) {
...
class _ActionRow extends StatelessWidget {
  ...
  Widget build(BuildContext context) => _RowShell(
      ...
      iconBackgroundColor:
          destructive ? AppColors.danger.withValues(alpha: .12) : null,
      trailing: action ??
