...
class HomeEntry {
  ...
  int get unreadCount => caughtUp
      ? 0
      : (chapters.length - currentIndex - 1).clamp(0, chapters.length).toInt();
...
final _homeCatalogProvider =
  ...
  for (final id in state.bookmarks) {
    try {
      ...
      final chapters = await repository.chapters(manga, refresh: true);
      if (chapters.isEmpty) continue;
      entries.add((manga: manga, chapters: chapters));
    } catch (_) {
      // One unavailable provider entry should not hide the rest of the home feed.
    }
  }
  return entries;
});
final homeEntriesProvider = Provider<AsyncValue<List<HomeEntry>>>((ref) {
  final progress =
      ref.watch(userLibraryProvider.select((value) => value.progress));
  return ref.watch(_homeCatalogProvider).whenData((catalog) {
    final entries = catalog.map((entry) {
      final mangaProgress = progress[entry.manga.id];
      final current = mangaProgress == null
          ? -1
          : entry.chapters
              .indexWhere((chapter) => chapter.id == mangaProgress.chapterId);
      final caught = current == entry.chapters.length - 1 &&
          (mangaProgress?.chapterProgress ?? 0) >= .95;
      final sort = caught
          ? (mangaProgress?.updatedAt ?? DateTime.fromMillisecondsSinceEpoch(0))
...
final _homeRefreshProvider = StreamProvider<int>((ref) =>
    Stream<int>.periodic(const Duration(minutes: 15), (value) => value));
...
class _HomeScreenState extends ConsumerState<HomeScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final caughtUpCount = homeEntries.maybeWhen(
        ...
        orElse: () => null);
    return Scaffold(
        appBar: AppBar(titleSpacing: 16, title: const _TsukiTitle(), actions: [
          Padding(
        ...
        body: Column(children: [
          ...
          Expanded(
              child: homeEntries.when(
                  ...
                  error: (_, __) =>
                      const _Empty('Home is unavailable right now.'),
                  data: (entries) {
                    final active =
      ...
      Row(mainAxisSize: MainAxisSize.min, children: [
        Container(
            ...
            decoration: BoxDecoration(
                color: AppColors.raised.withValues(alpha: .55),
                borderRadius: BorderRadius.circular(16)),
            child: ClipRRect(
                borderRadius: BorderRadius.circular(12),
                child: Image.asset('assets/branding/quiet_reader_icon.png',
                    fit: BoxFit.cover))),
                  ...
                  Text(countLabel,
                          style: TextStyle(
                              color: selected
                                  ...
                                  : AppColors.muted.withValues(alpha: .80),
                              fontSize: 12,
...
class _HomeCard extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final last = entry.currentIndex < 0
        ? 'Not started'
        : entry.chapters[entry.currentIndex].numberLabel;
                          ...
                          Expanded(
                              child: Column(
                                ...
                                _ChapterInfoLine(
                                    label: 'Last read',
                                    value: last,
                                    valueColor: entry.currentIndex < 0
                                        ? AppColors.muted
                                        : AppColors.accent),
                                const SizedBox(height: 2),
                                _ChapterInfoLine(
                                        ...
                                        style: FilledButton.styleFrom(
                                            ...
                                            shape: RoundedRectangleBorder(
                                                borderRadius: BorderRadius.circular(
                                                    buttonRadius))),
                                        onPressed: entry.caughtUp
                                            ? null
                                            : () => context.push(
                                                '/reader/${Uri.encodeComponent(entry.manga.id)}?chapter=${Uri.encodeComponent(entry.progress?.chapterId ?? entry.chapters.first.id)}'),
                                        child: Text(entry.progress == null
                                            ? 'Start Reading'
                                            : 'Continue Reading')))
                              ]))
                        ])))));
